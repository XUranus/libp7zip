...
struct CArcTime
{
    ...
    void Write_To_FiTime(CFiTime &dest) const {
        ...
        if (FILETIME_To_timespec(FT, dest)
            && ((Prec == k_PropVar_TimePrec_Base + 8 || Prec == k_PropVar_TimePrec_Base + 9) && Ns100 != 0)) {
            dest.tv_nsec += Ns100;
        }
     #endif
    }
...
static Func_CreateObject g_createObjectFunc = nullptr;

static Func_CreateObject GetCreateObjectProc(FString dllPrefix = FString(""))
{
    if (g_createObjectFunc) {
        return g_createObjectFunc;
    }
    ...
    if (!lib.Load(dllPrefix + FTEXT(kDllName))) {
        // Cannot load 7-zip library
        return nullptr;
    }

    g_createObjectFunc = (Func_CreateObject)lib.GetProc("CreateObject");
    if (!g_createObjectFunc)
    {
        // Cannot get CreateObject
        return nullptr;
    }
    return g_createObjectFunc;
}
...
bool SevenZip::Archive(
    const std::vector<std::string> &inputPaths,
    const std::string &archivePath)
{
    Func_CreateObject createObjectFunc = GetCreateObjectProc();
    if (createObjectFunc == nullptr) {
        return false;
    }

    FString archiveName(archivePath.c_str());
    UString password;
    bool passwordIsDefined = false;

    CObjectVector<CDirItem> dirItems;
    for (const std::string& inputPath : inputPaths) {
        FString name = FString(inputPath.c_str());
        NFind::CFileInfo fi;
        if (!fi.Find(name)) {
    ...
    return updateCallbackSpec->FailedFiles.Size() == 0;
}
